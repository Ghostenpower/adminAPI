<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['system:addresses:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['system:addresses:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:addresses:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:addresses:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="addressesList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="地址类型(1=用户地址,2=商家地址)" align="center" prop="addressType" />
      ...
      <el-table-column label="是否默认地址(0=否,1=是)" align="center" prop="isDefault" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="是否默认地址(0=否,1=是)" prop="isDefault">
          <el-input v-model="form.isDefault" placeholder="请输入是否默认地址(0=否,1=是)" />
        </el-form-item>
        <el-form-item label="创建时间" prop="createdAt">
          <el-date-picker clearable
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        addressType: [
          { required: true, message: "地址类型(1=用户地址,2=商家地址)不能为空", trigger: "change" }
        ],
        recipientName: [
        ...
        isDefault: [
          { required: true, message: "是否默认地址(0=否,1=是)不能为空", trigger: "blur" }
        ],
        status: [
          { required: true, message: "状态(0=禁用,1=正常)不能为空", trigger: "change" }
        ],
        createdAt: [
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      getAddresses(addressId).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除地址信息编号为"' + addressIds + '"的数据项？').then(function() {
        return delAddresses(addressIds)
      }).then(() => {
        this.getList()
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/addresses/export', {
        ...this.queryParams
