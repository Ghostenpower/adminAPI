...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        userType: [
          { required: true, message: "用户类型(1学生,2商家,3管理员)不能为空", trigger: "change" }
        ],
        status: [
          { required: true, message: "状态(0禁用,1正常)不能为空", trigger: "change" }
        ],
        createdAt: [
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除用户基本信息编号为"' + userIds + '"的数据项？').then(function() {
